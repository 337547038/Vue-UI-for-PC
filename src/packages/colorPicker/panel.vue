...
<template>
  <div ref="el" class="color-panel" :style="{background: background}" @mousedown="bgClick">
    ...
    <div class="color-cursor" :style="{left:style.left+'px',top:style.top+'px'}" @mousedown.stop="onMouseDown"></div>
  </div>
</template>
...
export default defineComponent({
  name: 'ColorPanel',
  props: {
    bgColor: pType.object(),
    showColor: pType.object()
  },
  setup(props) {
    const el = ref()
    ...
    // 通过右边界交点计算原点的坐标
    const calcHeightColor = (x: number, y: number, w: number, h: number, color: any) => {
